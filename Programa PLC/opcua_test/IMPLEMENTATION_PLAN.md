...
1. Crearemos una carpeta dedicada "InterfaceJetson" en el Modelo Optix.
2. Crearemos nodos Alias o Variable en esta carpeta que:
...
**Solución a Nivel de Código (NetLogic):** Si la UI no permite configurar NodeIds string arbitrarios fácilmente, usaremos un script NetLogic para registrar nodos con IDs específicos.
...
Vincular el `Value` de estas nuevas variables a los tags reales del PLC (ej: `CommDrivers/RAEtherNet_IPDriver1/Tags/Programming/EdgeGateway/...`).

### Paso 3: Configurar Servidor OPC UA

En **OPC UA > Server > Settings:**

1. Verificar los NamespaceUris. Agregar uno nuevo si es necesario, ej: `http://syngenta.com/jetson`.
2. Queremos que este quede mapeado al índice 4.

**Configuración de NodeId:**

1. Seleccionar la carpeta `edgegateway`.
2. Buscar propiedades "NodeId" en el panel de Propiedades.
3. **Acción Clave:** Usar funcionalidad de "Custom NodeManager" o "Alias" si los nodos estándar no soportan IDs String personalizados.
